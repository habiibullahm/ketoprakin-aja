...
export function CustomerAccount() {
  ...
  return <main className="min-h-screen bg-gray-50 p-4"><div className="mx-auto max-w-md space-y-4"><h1 className="text-2xl font-bold">Akun {name}</h1><section className="rounded-lg bg-primary p-5 text-primary-foreground"><p>Stempel loyalitas</p><p className="text-3xl font-bold">{stamps.stamps} / 10</p><p className="text-sm">{stamps.rewardAvailable ? "Gratis 1 porsi tersedia!" : `${stamps.remaining} pesanan lagi untuk gratis 1 porsi.`}</p></section><section className="rounded-lg bg-white p-4 shadow"><h2 className="font-semibold">Riwayat Pesanan</h2>{orders.length ? orders.map((order) => <Link className="mt-3 block rounded border p-3" key={order.id} to={`/customer/tracking?orderId=${order.id}`}><b>{order.orderNumber}</b><span className="float-right">{order.status}</span><p className="text-sm text-muted-foreground">Rp {Number(order.totalAmount).toLocaleString("id-ID")}</p></Link>) : <p className="mt-3 text-sm text-muted-foreground">Belum ada pesanan.</p>}</section></div></main>
}
